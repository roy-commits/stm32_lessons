...
/**
 * @brief 内部辅助函数：使能指定GPIO端口的时钟（抽象化实现）
 * @param port: 要使能时钟的GPIO端口（如GPIOA、GPIOB）
 * @retval 无
 */
static void GPIO_EnableClock(const GPIO_TypeDef* port) {
    if (port == NULL) return;

    // 遍历映射表，查找端口对应的时钟使能宏
    uint8_t mapSize = sizeof(g_gpioPortClockMap) / sizeof(GPIO_PortClockMapTypeDef);
